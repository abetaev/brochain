...
- **simplicity**: use the most direct strategy that satisfies current behavior. Push available data through immediately and omit or naturally fall back from unavailable data. When multiple designs are correct, choose the one with fewer states, branches, entities, and interactions. Do not introduce coordination phases, suppression bookkeeping, caches, queues, indirection, or generalized mechanisms unless a current requirement demonstrably needs them.
- **reactivity**: a state owner MUST publish each known domain change directly. Consumers apply that keyed change, with any consumer-specific transformation, to their existing projection. Use a full snapshot only for initialization or explicit recovery; do not publish generic invalidations that make consumers reload unchanged state when the changed data is already known.
...
- **lifetime ownership**: once owner shutdown begins, consumers MUST discard the owner and every dependency obtained from it and MUST NOT start new operations. Owners MUST provide idempotent shutdown and finish or cancel work accepted before shutdown; dependencies MUST NOT add defensive access checks for caller behavior prohibited by this convention.
...
- **semantic modeling**: diagrams describe architectural meaning, not required code entities. Do not translate a diagram label, shape, or relationship into a type or runtime object unless the implementation independently requires that contract.
...
- **documentation detail**: describe each architecture component with `dependencies` for prerequisites, `structure` for composed components, `use cases` for provided actions, and `behavior` for the flows and guarantees involving those actions. Add runtime and technologies only when they are non-obvious. Prefer short hierarchies, tables, and diagrams to prose. Do not narrate imports, filenames, call sequences, or facts evident from a shallow source reading; retain lower-level detail only for a non-obvious constraint, contract, security property, or operational decision.
